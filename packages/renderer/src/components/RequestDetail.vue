<template>
  ...
  <a-tabs v-model:activeKey="activeKey" type="card">
    ...
    <a-tab-pane key="4" tab="Cookie">{{ cookie }}</a-tab-pane>
  </a-tabs>
</template>

<script lang="ts">
  import { TableColumnsType } from "ant-design-vue";
import { ref, defineComponent, PropType, toRefs, watch, onMounted } from "vue";
  ...
  export default defineComponent({
    ...
    setup(props) {
      ...
      const formatData = (val: RequestData) => {
        if (val.headers) {
        ...
        body.value = val.body;
        cookie.value = val.headers?.cookie;
      };
